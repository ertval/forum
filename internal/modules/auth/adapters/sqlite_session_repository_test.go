...
func TestSQLiteSessionRepository_Create(t *testing.T) {
	...
	var id string
	err = db.QueryRow("SELECT id FROM sessions WHERE token = ?", session.Token).Scan(&id)
	if err != nil {
		t.Errorf("Session was not created in database: %v", err)
	}
	if id != session.ID {
		t.Errorf("Expected ID %s, got %s", session.ID, id)
	}
}
...
func TestSQLiteSessionRepository_GetByToken(t *testing.T) {
	...
	_, err = db.Exec("INSERT INTO sessions (id, user_id, token, expires_at, created_at, ip_address, user_agent) VALUES (?, ?, ?, ?, ?, ?, ?)",
		session.ID,
		session.UserID,
		session.Token,
		session.ExpiresAt,
	...
	if result.ID != session.ID {
		t.Errorf("Expected ID %s, got %s", session.ID, result.ID)
	}
...
func TestSQLiteSessionRepository_GetByUserID(t *testing.T) {
	...
	for _, session := range result {
		if session.UserID != userID {
			t.Errorf("Session %s has UserID %d, expected %d", session.ID, session.UserID, userID)
		}
	}
}
