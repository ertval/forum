...
func (m *mockModerationService) CreateReport(ctx context.Context, reporterID, targetID int, targetType, reason string) error {
	return nil
}

func (m *mockModerationService) ReviewReport(ctx context.Context, reportID int, decision string) error {
	return nil
}
...
func TestModerationServiceInterfaceMethods(t *testing.T) {
	...
	err := service.CreateReport(ctx, 1, 1, "post", "reason")
	if err != nil {
		// Expected to be not implemented in mock
	}
	
	err = service.ReviewReport(ctx, 1, "decision")
	if err != nil {
		// Expected to be not implemented in mock
	}
