...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
    -H "Content-Type: application/json" \
    -d '{"title":"Test Post","content":"Test content","categories":["general"]}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
    ...
    -d '{"title":"My First Post","content":"This is the content of my first post","categories":["general"]}')
END_TIME=$(date +%s%N)
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
    -H "Content-Type: application/json" \
    -H "Cookie: session_token=$SESSION_TOKEN" \
    -d '{"title":"","content":"Valid content","categories":["general"]}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
if [ "$HTTP_CODE" = "400" ]; then
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
    -H "Content-Type: application/json" \
    -H "Cookie: session_token=$SESSION_TOKEN" \
    -d '{"title":"Valid title","content":"","categories":["general"]}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
if [ "$HTTP_CODE" = "400" ]; then
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
    -H "Content-Type: application/json" \
    -H "Cookie: session_token=$SESSION_TOKEN" \
    -d "{\"title\":\"$LONG_TITLE\",\"content\":\"Valid content\",\"categories\":[\"general\"]}")
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
if [ "$HTTP_CODE" = "400" ]; then
    print_test "26" "PASS"
else
...
RESPONSE=$(curl -s -w "\n%{http_code}" "$BASE_URL/posts?category=general")
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
if [ "$HTTP_CODE" = "200" ]; then
    # Verify it returns posts (or empty array)
    if echo "$BODY" | grep -q "general\|posts\|\["; then
        print_test "30" "PASS"
    else
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
    -H "Content-Type: application/json" \
    -H "Cookie: session_token=$SESSION_TOKEN" \
    -d '{"title":"Post to Delete","content":"This post will be deleted","categories":["general"]}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
    ...
    -d '{"title":"User1 Post","content":"Content from user 1","categories":["general"]}')
HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
BODY=$(echo "$RESPONSE" | sed '$d')
...
if [ -n "$SESSION_TOKEN1_NEW" ] && [ -n "$SESSION_TOKEN2" ] && [ -n "$SESSION_TOKEN3" ]; then
    ...
    (curl -s -X POST "$BASE_URL/posts" \
        ...
        -d '{"title":"Concurrent Post 1","content":"From user 1","categories":["general"]}' > /dev/null 2>&1) &
    PID1=$!
    
    (curl -s -X POST "$BASE_URL/posts" \
        -H "Content-Type: application/json" \
        -H "Cookie: session_token=$SESSION_TOKEN2" \
        -d '{"title":"Concurrent Post 2","content":"From user 2","categories":["general"]}' > /dev/null 2>&1) &
    PID2=$!
    
    (curl -s -X POST "$BASE_URL/posts" \
        -H "Content-Type: application/json" \
        -H "Cookie: session_token=$SESSION_TOKEN3" \
        -d '{"title":"Concurrent Post 3","content":"From user 3","categories":["general"]}' > /dev/null 2>&1) &
    PID3=$!
...
if [ -n "$SESSION_TOKEN1_NEW" ]; then
    ...
    for i in {1..10}; do
        RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
            -H "Content-Type: application/json" \
            -H "Cookie: session_token=$SESSION_TOKEN1_NEW" \
            -d "{\"title\":\"Bulk Post $i\",\"content\":\"Content for bulk post $i\",\"categories\":[\"general\"]}")
        HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
        if [ "$HTTP_CODE" = "201" ]; then
...
if [ -n "$SESSION_TOKEN3" ]; then
    ...
    for i in {1..30}; do
        RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
            -H "Content-Type: application/json" \
            -H "Cookie: session_token=$SESSION_TOKEN3" \
            -d "{\"title\":\"Rate Test $i\",\"content\":\"Testing rate limits\",\"categories\":[\"general\"]}" 2>/dev/null)
        HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
        if [ "$HTTP_CODE" = "429" ]; then
            RATE_LIMIT_HIT=true
            break
...
if [ -n "$SESSION_TOKEN1_NEW" ]; then
    ...
    RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
        -H "Content-Type: application/json" \
        -H "Cookie: session_token=$SESSION_TOKEN1_NEW" \
        -d "{\"title\":\"Large Content Post\",\"content\":\"$LARGE_CONTENT\",\"categories\":[\"general\"]}" 2>/dev/null)
    HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
...
if [ -n "$SESSION_TOKEN1_NEW" ]; then
    RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
        -H "Content-Type: application/json" \
        -H "Cookie: session_token=$SESSION_TOKEN1_NEW" \
        -d '{"title":"Test'\'' OR 1=1; DROP TABLE posts; --","content":"Testing SQL injection","categories":["general"]}')
    HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
...
if [ -n "$SESSION_TOKEN1_NEW" ]; then
    RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
        -H "Content-Type: application/json" \
        -H "Cookie: session_token=$SESSION_TOKEN1_NEW" \
        -d '{"title":"XSS Test","content":"<script>alert(\"XSS\")</script>","categories":["general"]}')
    HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
    BODY=$(echo "$RESPONSE" | sed '$d')
    if [ "$HTTP_CODE" = "201" ] || [ "$HTTP_CODE" = "400" ]; then
        print_test "76" "PASS"
...
if [ -n "$SESSION_TOKEN1_NEW" ]; then
    RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
        ...
        -H "Cookie: session_token=$SESSION_TOKEN1_NEW" \
        -d '{"title":"Unicode Test 你好 🎉","content":"Testing émojis 🚀 and spëcial çharacters","categories":["general"]}')
    HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
    if [ "$HTTP_CODE" = "201" ]; then
        print_test "77" "PASS"
    else
...
if [ -n "$SESSION_TOKEN1_NEW" ]; then
    RESPONSE=$(curl -s -w "\n%{http_code}" -X POST "$BASE_URL/posts" \
        -H "Content-Type: application/json" \
        -H "Cookie: session_token=$SESSION_TOKEN1_NEW" \
        -d $'{"title":"Null\x00Test","content":"Content with null byte","categories":["general"]}' 2>/dev/null)
    HTTP_CODE=$(echo "$RESPONSE" | tail -n1)
